...
[LayersCollection](http://edward17.github.io/LayersCollection/) - сборник растровых слоёв, основанных на данных OpenStreetMap, и слоёв, которые могут использоваться для редактирования OSM. К ним относятся:

* Спутниковые снимки
* GPS-треки
* Карта покрытия панорам [Mapillary](http://www.mapillary.com/)

Также присутсвуют слои рельефа.
...
Также вместо базового слоя можно сделать фон чёрным (Set black background) или серым (Set gray background).
...
Категория - это слои под заголовком (до другого заголовка). Все слои в одной категории относятся к одной теме. Например, в категории "General" собраны слои без уклона на что-либо (они показывают всё сразу, грубо говоря). В категории "Public Transport" - как можно догадатся из названия, находятся слои, отображающие маршруты общественного транспорта.
...
Благодаря этому можно быстро переходить с одного сайта на другой. Например, вы увидели  ошибку в одном из валидаторов http://pavlo-dudka.github.io/ , и хотите посмотреть, как выглядит данное место на спутниковых снимках. Порядок действий следующий:
...
Есть возможность показывать собственный тайловый слой. Для этого нажмите на кнопку "Add custom layer" и заполните поля в появившейся форме. Обязательные поля отмечены `*`. После чего нажмите кнопку "Show this layer".
...
1. Данные этого слоя (адрес, копирайты, максимальный зум) не сохраняются и даже при перезагрузке страницы будут потеряны
2. С помощью этого инструмента **нельзя** попросить автора (меня) добавить новый слой в сборник. Для этого нужно связаться со мной. Об этом написано в разделе "Контакты"
...
3. Теперь можно скопировать результат
...
* Показывать ли левую панель? (На ней расположны фильтры, список слоёв и многое другое)
* Показывать ли фильтры?
* Каждый фильтр
* Показ каждой категории
