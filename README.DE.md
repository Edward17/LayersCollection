...
[LayersCollection](http://edward17.github.io/LayersCollection/) ist eine Versammlung von raster OpenStreetMap-basierenden Layers und Layers, die man zur Editierung von OSM benutzen darf. Dazu gehören:

* Luftbilder
* GPS Tracks

Eg gibt auch Layers, die den Relief darstellen, und Karte von [Mapillary](http://www.mapillary.com/) Panoramen.

# Ziel

Deiser Projekt ist geschaft, um:
...
* Baselayer - Layer, der im Kartehintergrung liegt; Layer, über dem alles anderes aufgestapelt wird
* Overlay - Layer mit transparentem Hintergrung, der über Baselayer aufgestappelt werden kann. Gleichzeitige Vorführung von mehreren Overlays ist möglich. Sie werden in der Reihenfolge geyeigt, in der sie eingeschaltet wurden

Auch kann man den Hintergrund schwarz ("Set black background" Taste) oder grau ("Set gray background") machen.

Wenn viele Overlays gezeigt werden und alle zusammen verstecket werden müssen, kann man, statt lange die Liste scrollen und Checkboxe ausschalten, kann man einfach "Remove all overlays" Taste drucken.
...
Kategorie ist Layers unter einem Überschrift (bis zum nächsten Überschrift). Alle Layers in einer Kategorie gehören zum dasselben Thema. Zum Beispiel, Layers in der "General" Kategorie sind ohne Spezialisierung (sie zeigen alles sofort). In der "Public Transport" Kategorie sind Layers, die Linien von öffentlichen Verkehrsmitteln darstellen, versammelt.

Kategorie kann verbergt werden, damit Layersliste kürzer wird, und Zeit von Listescrolling - kleiner. Dafür muss man den Checkbox links von dem Layername drucken.
...
Die Website unterstützt (d. h. versteht) volgende Hashformaten:
...
Im Hash von http://www.openstreetmap.org/ Webseite werden nicht nur Koordinaten, sondern auch gewählte Layer gespeichert. Wenn man solchen Hash einsetzt, wird diese Layer automatish gezeigt.
...
Momentan sind auf dieser Werbseite viele verschiedene Layers versammelt. Um die Suche des Layers nach bestimmten Kriterien zu erleichtern, kann man Filters benutzen:

* Language (Sprachcode mit grünem Hintergrund) - Sprache der Aufschriften
* Show outdated layers (красный "old") - zeigen Layers, die lange Zeit nicht erneut wurden
* Show Ukraine-only layers (жёлтый "ua") - zeigen Layers, die nur die Ukraine bedecken. Dieses Filter ist aktuell für Bewohner von anderen Ländern, die in diesen Layers nichts Interessantes finden können
* Show only black/white layers (серый "b/w") - zeigen **nur** schwarzweiße Layers. Sie können z. B. für Druck benutzt werden
* Show only 3D-buildings layers (фиолетовый "3D") - zeigen **nur** Layers, die in höhen Zoomen die Gebeuden in 3D darstellen
* Show only Retina layers (оранжевый "rtn")  - zeigen **nur** Layers für Retina-Displays
* Show only no-labels layers (тёмно-синий "nl") - zeigen **nur** Layers, die keine Aufschriften haben. Sie können z. B. als Hintergrund für Karte mit eigenen Aufschriften benutzt werden

Gleichzeitige Aktivierung von mehreren Filters ist möglich.
...
1. Daten von diesem Layer (Adresse, Autor, maximal Zoom) werden nicht gespeichert und werden sogar bei der Auffrischung von Seite verloren
2. Mithilfe von diesem Instrument kann man *nicht* den Autor (mich) bitten, einen neuen Layer in Versammlung einzufügen. Dafür muss man mir schreiben, darüber geht es im "Kontakten" Abschnitt
...
* Редактор iD - dafür wird die Adresse, die man bei Wahl von eigenen Layer angeben muss, gezeigt
* Редактор JOSM - dafür wird die Adresse und maximal Zoom erstellt (für TMS layer) oder die Adresse, Layersliste und Format (für WMS layer) gezeigt
...
3. Nun kann man den Ergebniss kopieren
...
* Ob die linke Panel gezeigt wird? (Dort sind Filters, Layersliste und alles andere)
* Ob die Filters gezeigt werden?
* Jeder Filter
